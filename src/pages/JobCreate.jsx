...
export default function JobCreate() {
    ...
    const submit = async (e) => {
        ...
        await api.createJob({
            ...form,
            customerName: form.customerName || (customers.find(c => c.id === Number(form.customerId)) || {}).name
        })
        nav('/jobs')
    }
        ...
        <DashboardLayout>
            ...
            <form onSubmit={submit} className="grid md:grid-cols-2 gap-4 bg-white p-4 rounded shadow">
                ...
                <input value={form.vehicle} onChange={(e) => setForm({...form, vehicle: e.target.value})}
                       placeholder="Vehicle (plate/model)" className="p-2 border rounded"/>
                <input type="number" value={form.estimate}
                       onChange={(e) => setForm({...form, estimate: Number(e.target.value)})} placeholder="Estimate"
                       className="p-2 border rounded"/>
                <textarea value={form.notes} onChange={(e) => setForm({...form, notes: e.target.value})}
                          placeholder="Notes" className="p-2 border rounded md:col-span-2"/>
